...
public class BTree<Key extends Comparable<Key>, Value>
{
	...
	private static class Entry
	{
		...
		public Entry(final Comparable key, final Object value, final Node next)
		{
			this.key = key;
			this.value = value;
			this.next = next;
	...
	{
		private final Entry[]	children	= new Entry[M]; // the array of
															// children
		private int				m;							// number of
															// children
	...
	private static final int	M	= 4;	// max children per B-tree node =
											// M-1
	...
	private Node insert(final Node h, final Key key, final Value value,
			final int ht)
	{
		int j;
		...
		else
		{
			for (j = 0; j < h.m; j++)
			{
				...
				{
					final Node u = insert(h.children[j++].next, key, value,
							ht - 1);
					if (u == null)
					{
						return null;
	...
	{
		final Node u = insert(root, key, value, HT);
		N++;
		if (u == null)
		{
			return;
