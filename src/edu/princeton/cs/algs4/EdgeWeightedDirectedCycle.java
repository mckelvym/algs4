...
public class EdgeWeightedDirectedCycle
{
	...
	private Stack<DirectedEdge>		cycle;		// directed cycle (or null if no
												// such cycle)
	private final DirectedEdge[]	edgeTo;	// edgeTo[v] = previous edge on
												// path to v
	private final boolean[]			marked;	// marked[v] = has vertex v been
												// marked?

	private final boolean[]			onStack;	// onStack[v] = is vertex on the
												// stack?
	...
	private boolean check(final EdgeWeightedDigraph G)
	{
		...
		if (hasCycle())
		{
			...
			for (final DirectedEdge e : cycle())
			{
				if (first == null)
				{
					first = e;
