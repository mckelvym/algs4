...
public class FloydWarshall
{
	...
	public static void main(final String[] args)
	{
		...
		final AdjMatrixEdgeWeightedDigraph G = new AdjMatrixEdgeWeightedDigraph(
				V);
		for (int i = 0; i < E; i++)
		{
			...
			final double weight = Math
					.round(100 * (StdRandom.uniform() - 0.15)) / 100.0;
			if (v == w)
			{
	...
	private double[][]			distTo;			// distTo[v][w] = length of
													// shortest v->w path
	private DirectedEdge[][]	edgeTo;			// edgeTo[v][w] = last edge
													// on shortest v->w path

	private boolean				hasNegativeCycle;	// is there a negative
													// cycle?
