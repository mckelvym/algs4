 ...
 * 4th Edition</i> by Robert Sedgewick and Kevin Wayne. For alternate
 * implementations, see {@link LazyPrimMST}, {@link KruskalMST}, and
 * {@link BoruvkaMST}.
 *
...
public class PrimMST
{
	...
	private final double[]				distTo; // distTo[v] = weight of
												// shortest such edge
	private final Edge[]				edgeTo; // edgeTo[v] = shortest edge
												// from tree vertex to non-tree
												// vertex
	private final boolean[]				marked; // marked[v] = true if v on
	...
	private boolean check(final EdgeWeightedGraph G)
	{
		...
		for (final Edge e : edges())
		{
			...
			uf = new UF(G.V());
			for (final Edge f : edges())
			{
				final int x = f.either(), y = f.other(x);
				if (f != e)
				{
					uf.union(x, y);
				}
