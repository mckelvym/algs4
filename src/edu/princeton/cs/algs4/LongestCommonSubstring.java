...
public class LongestCommonSubstring
{
	...
	public static void main(final String[] args)
	{
		...
		final String text1 = in1.readAll().trim().replaceAll("\\s+", " ");
		final String text2 = in2.readAll().trim().replaceAll("\\s+", " ");
		final int N1 = text1.length();
		// int N2 = text2.length();

		// concatenate two string with intervening '\1'
		final String text = text1 + '\1' + text2;
		final int N = text.length();

		// compute suffix array of concatenated text
		final SuffixArray suffix = new SuffixArray(text);
