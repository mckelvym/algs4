...
/**
 * The <code>PatriciaSET</code> class provides an implementation of an unordered
 * set, with the restriction that the items (keys) are of class
...
public class PatriciaSET implements Iterable<String>
{
	 ...
	 * bits. Notice that the very first character comparison excludes the
	...
	/**
	 * Unit tests the <code>PatriciaSET</code> data type. This test fixture runs
	 * a series of tests on a randomly generated dataset. You may specify up to
	 ...
	 * required) to extend (logically) the string beyond its length. The inner
	 * loops of the get and put methods flow much better when they are not
	 * concerned with the lengths of strings, so a trick is employed to allow
	 * the get and put methods to view every string as an "infinite" sequence of
	 * bits. Logically, every string gets a '\uffff' character, followed by an
	 * "infinite" sequence of '\u0000' characters, appended to the end. Note
	 * that the '\uffff' character serves to mark the end of the string, and it
	 * is necessary. Simply padding with '\u0000' is insufficient to make all
	 * unique Unicode strings "look" unique to the get and put methods (because
	 * these methods do not regard string lengths).
	 */
	 ...
	 *             if <code>key</code> is the empty string.
	 */
	 ...
	 * of the keys in a set named <code>set</code>, use the foreach notation:
	 * <code>for (Key key : set)</code>.
	 * 
