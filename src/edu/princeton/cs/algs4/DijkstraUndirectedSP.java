 ...
 * the single-source shortest paths problem in edge-weighted graphs where the
...
public class DijkstraUndirectedSP
{
	...
	private final double[]				distTo; // distTo[v] = distance of
												// shortest s->v path
	private final Edge[]				edgeTo; // edgeTo[v] = last edge on
												// shortest s->v path
	...
	private boolean check(final EdgeWeightedGraph G, final int s)
	{
		...
		for (int w = 0; w < G.V(); w++)
		{
			...
			final Edge e = edgeTo[w];
			if (w != e.either() && w != e.other(e.either()))
			{
				return false;
			}
			final int v = e.other(w);
			if (distTo[v] + e.weight() != distTo[w])
			{
	...
	/**
	 * Returns a shortest path between the source vertex <tt>s</tt> and vertex
	 * <tt>v</tt>.
	 *
	 * @param v
	 *            the destination vertex
	 * @return a shortest path between the source vertex <tt>s</tt> and vertex
	 *         <tt>v</tt>; <tt>null</tt> if no such path
	 */
