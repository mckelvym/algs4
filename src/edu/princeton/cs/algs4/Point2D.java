...
public final class Point2D implements Comparable<Point2D>
{
	...
	private static class ROrder implements Comparator<Point2D>
	{
		...
		public int compare(final Point2D p, final Point2D q)
		{
			final double delta = p.x * p.x + p.y * p.y
					- (q.x * q.x + q.y * q.y);
			if (delta < 0)
			{
				return -1;
	...
	{
		final double area2 = (b.x - a.x) * (c.y - a.y) - (b.y - a.y)
				* (c.x - a.x);
		if (area2 < 0)
		{
			return -1;
	 ...
	 * @param other
	 *            the other point
	 * @return <tt>true</tt> if this point equals <tt>other</tt>; <tt>false</tt>
	 *         otherwise
	 */
	@Override
	...
	public int hashCode()
	{
		final int hashX = ((Double) x).hashCode();
		final int hashY = ((Double) y).hashCode();
