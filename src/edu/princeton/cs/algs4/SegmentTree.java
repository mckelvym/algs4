...
public class SegmentTree
{
	...
	public static void main(final String[] args)
	{
		...
		while (true)
		{
			...
			{
				StdOut.printf("Sum from %d to %d = %d%n", arg1, arg2,
						st.RSQ(arg1, arg2));
			}
			else if (line[0].equals("rmq"))
			{
				StdOut.printf("Min from %d to %d = %d%n", arg1, arg2,
						st.RMinQ(arg1, arg2));
			}
			else
			{
	...
	public SegmentTree(final int[] array)
	{
		...
		size = (int) (2 * Math.pow(2.0,
				Math.floor(Math.log(array.length) / Math.log(2.0) + 1)));
		heap = new Node[size];
	...
	private boolean contains(final int from1, final int to1, final int from2,
			final int to2)
	{
	...
	// range2[from2, to2]
	private boolean intersects(final int from1, final int to1, final int from2,
			final int to2)
	{
	...
	private int RMinQ(final int v, final int from, final int to)
	{
		...
		{
			propagate(v);
			final int leftMin = RMinQ(2 * v, from, to);
			final int rightMin = RMinQ(2 * v + 1, from, to);
	...
	private int RSQ(final int v, final int from, final int to)
	{
		...
		{
			propagate(v);
			final int leftSum = RSQ(2 * v, from, to);
			final int rightSum = RSQ(2 * v + 1, from, to);
	...
	private void update(final int v, final int from, final int to,
			final int value)
	{
