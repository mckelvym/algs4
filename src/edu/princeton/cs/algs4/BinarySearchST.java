 ...
 * <em>maximum</em>, <em>floor</em>, <em>select</em>, and <em>ceiling</em>. It
 * also provides a <em>keys</em> method for iterating over all of the keys. A
 * symbol table implements the <em>associative array</em> abstraction: when
 * associating a value with a key that is already in the symbol table, the
 * convention is to replace the old value with the new value. Unlike
 * {@link java.util.Map}, this class uses the convention that values cannot be
 * <tt>null</tt>&mdash;setting the value associated with a key to <tt>null</tt>
 ...
 * implements the <tt>Comparable</tt> interface and calls the
 * <tt>compareTo()</tt> and method to compare two keys. It does not call either
 * <tt>equals()</tt> or <tt>hashCode()</tt>. The <em>put/em> and <em>remove</em>
 * operations each take linear time in the worst case; the <em>contains</em>,
 * <em>ceiling</em>, <em>floor</em>, and <em>rank</em> operations take
 * logarithmic time; the <em>size</em>, <em>is-empty</em>, <em>minimum</em>,
 * <em>maximum</em>, and <em>select</em> operations take constant time.
