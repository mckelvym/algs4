...
public class FibonacciMinPQ<Key> implements Iterable<Key>
{
	...
	private final HashMap<Integer, Node>	table	= new HashMap<Integer, Node>(); // Used
																					// for
																					// the
																					// consolidate
																					// operation
	...
	public Key delMin()
	{
		...
		head = cut(min, head);
		final Node x = min.child;
		final Key key = min.key;
		min.key = null;
		if (x != null)
		{
			head = meld(head, x);
