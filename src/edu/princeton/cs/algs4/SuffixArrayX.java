...
public class SuffixArrayX
{
	...
	public static void main(final String[] args)
	{
		...
		for (int i = 0; check && i < s.length(); i++)
		{
			if (suffix1.index(i) != suffix2.index(i))
			{
				...
				final String ith = "\""
						+ s.substring(suffix1.index(i),
								Math.min(suffix1.index(i) + 50, s.length()))
						+ "\"";
				final String jth = "\""
						+ s.substring(suffix2.index(i),
	...
	private final int[]		index;	// index[i] = j means text.substring(j) is
									// ith largest suffix
	...
	public int rank(final String query)
	{
		...
		while (lo <= hi)
		{
			...
			final int cmp = compare(query, index[mid]);
			if (cmp < 0)
			{
				hi = mid - 1;
			}
	...
	private void sort(final int lo, final int hi, final int d)
	{
		...
		int lt = lo, gt = hi;
		final char v = text[index[lo] + d];
		int i = lo + 1;
		while (i <= gt)
		{
			final char t = text[index[i] + d];
			if (t < v)
			{
				exch(lt++, i++);
			}
