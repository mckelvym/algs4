...
public class MultiwayMinPQ<Key> implements Iterable<Key>
{
	 ...
	 *             if D < 2
	 */
	public MultiwayMinPQ(final Comparator<Key> C, final int D)
	{
		if (D < 2)
		{
	 ...
	 *             if D < 2
	 */
	public MultiwayMinPQ(final Comparator<Key> C, final Key[] a, final int D)
	{
		if (D < 2)
		{
			throw new IllegalArgumentException("Dimension should be 2 or over");
	...
	public Key delMin()
	{
		...
		exch(0, --n);
		sink(0);
		final Key min = keys[n + d];
		keys[n + d] = null;
		final int number = getN(order - 2);
		if (order > 1 && n == number)
		{
