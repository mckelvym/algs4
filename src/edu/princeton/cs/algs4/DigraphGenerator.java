...
public class DigraphGenerator
{
	...
	/**
	 * Returns a random simple DAG containing <tt>V</tt> vertices and <tt>E</tt>
	 ...
	 * @param E
	 *            the number of vertices
	 * @return a random simple DAG on <tt>V</tt> vertices, containing a total of
	 *         <tt>E</tt> edges
	...
	/**
	 * Returns a random rooted-in DAG on <tt>V</tt> vertices and <tt>E</tt>
	 * edges. A rooted in-tree is a DAG in which there is a single vertex
	 * reachable from every other vertex. The DAG returned is not chosen
	 ...
	 * @param E
	 *            the number of edges
	 * @return a random rooted-in DAG on <tt>V</tt> vertices and <tt>E</tt>
	 *         edges
	 */
	...
	/**
	 * Returns a random rooted-in tree on <tt>V</tt> vertices. A rooted in-tree
	...
	/**
	 * Returns a random rooted-out DAG on <tt>V</tt> vertices and <tt>E</tt>
	 ...
	 * @param E
	 *            the number of edges
	 * @return a random rooted-out DAG on <tt>V</tt> vertices and <tt>E</tt>
	 *         edges
	 */
	...
	/**
	 * Returns a random rooted-out tree on <tt>V</tt> vertices. A rooted
	 * out-tree is an oriented tree in which each vertex is reachable from a
	 * single vertex. It is also known as a <em>arborescence</em> or
	...
	/**
	 * Returns a random simple digraph on <tt>V</tt> vertices, with an edge
	 ...
	 * takes time propotional to V^2 (even if <tt>p</tt> is small).
	 * 
	 * @param V
	 *            the number of vertices
	 * @param p
	 *            the probability of choosing an edge
	 * @return a random simple digraph on <tt>V</tt> vertices, with an edge
	...
	/**
	 * Returns a random simple digraph containing <tt>V</tt> vertices and
	 * <tt>E</tt> edges.
	 * 
	 * @param V
	 *            the number of vertices
	 * @param E
	 *            the number of vertices
	 * @return a random simple digraph on <tt>V</tt> vertices, containing a
	...
	/**
	 * Returns a random simple digraph on <tt>V</tt> vertices, <tt>E</tt> edges
	 * and (at least) <tt>c</tt> strong components. The vertices are randomly
	 * assigned integer labels between <tt>0</tt> and <tt>c-1</tt>
	 * (corresponding to strong components). Then, a strong component is creates
	 * among the vertices with the same label. Next, random edges (either
	 * between two vertices with the same labels or from a vetex with a smaller
	 * label to a vertex with a larger label). The number of components will be
	 * equal to the number of distinct labels that are assigned to vertices.
	 *
	 * @param V
	 *            the number of vertices
	 * @param E
	 ...
	 * @return a random simple digraph on <tt>V</tt> vertices and <tt>E</tt>
	 *         edges, with (at most) <tt>c</tt> strong components
	 * @throws IllegalArgumentException
	 *             if <tt>c</tt> is larger than <tt>V</tt>
	 */
	public static Digraph strong(final int V, final int E, final int c)
	{
		...
		for (int i = 0; i < c; i++)
		{
			...
			// if (count == 0)
			// System.err.println("less than desired number of strong components");
			...
			{
				final int w = StdRandom.uniform(v + 1, count);
				final Edge e = new Edge(w, v);
				set.add(e);
				G.addEdge(vertices[w], vertices[v]);
			}
	...
	// tournament
	/**
	 * Returns a random tournament digraph on <tt>V</tt> vertices. A tournament
