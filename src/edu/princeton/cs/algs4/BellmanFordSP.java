...
public class BellmanFordSP
{
	...
	private Iterable<DirectedEdge>	cycle;		// negative cycle (or null if no
												// such cycle)
	private final double[]			distTo;	// distTo[v] = distance of
												// shortest s->v path
	private final DirectedEdge[]	edgeTo;	// edgeTo[v] = last edge on
												// shortest s->v path
	private final boolean[]			onQueue;	// onQueue[v] = is v currently
												// on the queue?
	...
	private boolean check(final EdgeWeightedDigraph G, final int s)
	{
		...
		{
			double weight = 0.0;
			for (final DirectedEdge e : negativeCycle())
			{
				weight += e.weight();
			}
			if (weight >= 0.0)
	...
	private void findNegativeCycle()
	{
		...
		final EdgeWeightedDirectedCycle finder = new EdgeWeightedDirectedCycle(
				spt);
		cycle = finder.cycle();
	}
